...
/// <summary>Redacted native management-plane evidence; native states never establish runtime admission.</summary>
...
/// <summary>One completed, attributable management-plane capture; not a runtime readiness assertion.</summary>
...
/// <param name="CompletedAtUtc">Capture completion time; individual observations retain request completion times.</param>
...
public sealed class AzureInfrastructureCollector
{
    ...
    public async Task<AzureInfrastructureCollection> CollectAsync(
        ...
        TimeSpan requestTimeout, CancellationToken cancellationToken = default)
    {
        ...
        await Parallel.ForEachAsync(Enumerable.Range(0, groups.Length),
            ...
            async (index, token) => results[index] = await ReadAsync(groups[index].Key, requestTimeout, token));
        cancellationToken.ThrowIfCancellationRequested();
        ...
        for (var index = 0; index < groups.Length; index++)
        {
            ...
            foreach (var binding in groups[index])
            {
                var observation = new InfrastructureResourceObservation(binding.PhysicalResource,
                    ExecutionHealthStatus.Unknown, ExecutionReadinessStatus.Unknown, result.At,
                    [SourceReference.Create("azure-arm-get", result.ApiVersion ?? "unsupported")]);
                var evidence = new AzureInfrastructureEvidence(binding,
                    result.Error is null ? AzureInfrastructureEvidenceKind.Provisioning : AzureInfrastructureEvidenceKind.CollectionFailed,
                    observation);
                resources.Add(new(evidence, result.ProvisioningState, result.SiteState,
                    result.Error is null ? [] : [new DocumentValidationDiagnostic(
                        ...
                        Message: "Azure management evidence could not be admitted; inspect identity, access and collector coverage.",
                        Location: "/resources", SchemaLocation: binding.PhysicalResource.Value)]));
            }
        }
        cancellationToken.ThrowIfCancellationRequested();
    ...
    async Task<NativeResult> ReadAsync(string resourceId, TimeSpan timeout, CancellationToken cancellationToken)
    {
        var segments = resourceId.Split('/');
        var resourceType = segments[6] + "/" + string.Join('/', segments.Where((_, i) => i >= 7 && i % 2 == 1));
        var version = resourceType.ToLowerInvariant() switch
        {
        ...
        try
        {
            ...
            if (!root.TryGetProperty("id", out var id) || id.ValueKind != JsonValueKind.String
                || !string.Equals(id.GetString(), resourceId, StringComparison.OrdinalIgnoreCase)
                || !root.TryGetProperty("type", out var type) || type.ValueKind != JsonValueKind.String
                || !string.Equals(type.GetString(), resourceType, StringComparison.OrdinalIgnoreCase)) return Failure("identityMismatch");
