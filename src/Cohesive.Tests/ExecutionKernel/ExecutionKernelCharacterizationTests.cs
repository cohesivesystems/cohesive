...
public sealed class ExecutionKernelCharacterizationTests
{
    ...
    [
        ...
        new("EK-02", KernelScenarioStatus.Partial, "Canonical Process IR now has a closed typed AwaitMatch with stable clauses, guards, timers, exact interaction references, deterministic arbitration, and explicit late, stale, duplicate, and missing-target policy; durable registration, inbox claims, winner consumption, and Process runtime integration do not exist."),
        ...
        new("EK-04", KernelScenarioStatus.Partial, "Canonical Process IR now has stable Fork branches, reciprocal Joins, and explicit completion, failure, cancellation, ordering, and tie-break policy; the legacy runtime still has one cursor and no durable token or Join realization."),
