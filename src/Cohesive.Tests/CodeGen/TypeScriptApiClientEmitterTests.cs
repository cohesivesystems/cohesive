...
public sealed class TypeScriptApiClientEmitterTests
{
    ...
    [Fact]
    public void Emit_ProjectsHttpWireNamesToValidUniqueTypeScriptIdentifiers()
    {
        var definition = Cohesive.Api.Api.Define("Processes")
            .Action("Start")
                .Route("POST", "/api/processes/{123-id}")
                .RouteParameter<string>("123-id")
                .HeaderParameter<string>("Ari-Process-Key")
                .HeaderParameter<string>("Idempotency-Key")
                .OptionalHeaderParameter<string>("Ari-Command-Id")
                .HeaderParameter<string>("X-Mode")
                .HeaderParameter<string>("xMode")
        ...
        var document = Assert.Single(new TypeScriptApiClientEmitter(new()
        {
            NewLine = "\n",
            EmitAutoGeneratedHeader = false
        }).Emit(definition).Documents);

        Assert.Contains(
            "export function start(http: ApiHttpClient, _123Id: string, ariProcessKey: string, "
            + "idempotencyKey: string, ariCommandId: string | null | undefined, xMode: string, xMode2: string, "
            + "_default: string, body: DispatchShipmentRequest): Promise<ShipmentDto>",
            document.Text,
            StringComparison.Ordinal);
        Assert.Contains(
            "const basePath = `/api/processes/${encodeURIComponent(String(_123Id))}`;",
            document.Text,
            StringComparison.Ordinal);
        Assert.Contains("headers['Ari-Process-Key'] = String(ariProcessKey);", document.Text, StringComparison.Ordinal);
        Assert.Contains("headers['Idempotency-Key'] = String(idempotencyKey);", document.Text, StringComparison.Ordinal);
        Assert.Contains("headers['Ari-Command-Id'] = String(ariCommandId);", document.Text, StringComparison.Ordinal);
        Assert.Contains("headers['X-Mode'] = String(xMode);", document.Text, StringComparison.Ordinal);
        Assert.Contains("headers['xMode'] = String(xMode2);", document.Text, StringComparison.Ordinal);
        Assert.Contains("queryParams.set('default', String(_default));", document.Text, StringComparison.Ordinal);
        ...
        Assert.Contains("readonly start: { readonly _123Id: string; };", mock.Text, StringComparison.Ordinal);
        Assert.Contains("readonly start: { readonly _default?: string; };", mock.Text, StringComparison.Ordinal);
        Assert.Contains("route: { _123Id: route0['123-id'] } as ProcessApiRouteByEndpoint['start']", mock.Text, StringComparison.Ordinal);
        Assert.Contains("query._default = _default as string;", mock.Text, StringComparison.Ordinal);
    }
