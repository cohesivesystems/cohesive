...
/// <summary>
/// Validates identity-free observation fields against shape semantics.
/// </summary>
public static class ObservationValidator
{
    ...
    /// <summary>
    /// Validates that a concrete object value adheres to the supplied shape semantics.
    /// </summary>
        ...
        ShapeGraph? graph = null)
    {
        ...
        {
            validationError = "An observation must be a concrete, present, non-null object value.";
            return false;
        }

        if (!TryValidatePortableValueTree(value, out validationError))
            return false;

        return TryValidateAgainstShape(value.Fields, shape, out validationError, graph);
    }

    /// <summary>
    /// Validates that canonical observation fields adhere to the supplied shape semantics.
    /// </summary>
    ...
    static bool TryValidatePortableValueTree(
        ObservationValue root,
        out string? validationError)
    {
        Stack<(ObservationValue Value, string Path)> pending = new();
        pending.Push((root, "$"));
        while (pending.TryPop(out var item))
        {
            if (item.Value.Kind == ObservationValueKind.Undefined)
            {
                validationError =
                    $"An observation cannot contain an undefined value at '{item.Path}'. Omit an absent optional field instead.";
                return false;
            }

            if (item.Value.Kind == ObservationValueKind.Double && !double.IsFinite(item.Value.Double))
            {
                validationError = $"An observation cannot contain a non-finite number at '{item.Path}'.";
                return false;
            }

            if (item.Value.Kind == ObservationValueKind.Object && item.Value.Fields is not null)
            {
                foreach (var (name, child) in item.Value.Fields.OrderByDescending(
                             static field => field.Key,
                             StringComparer.Ordinal))
                {
                    pending.Push((child, $"{item.Path}.{name}"));
                }
            }
            else if (item.Value.Kind == ObservationValueKind.Array && !item.Value.Array.IsDefault)
            {
                for (var index = item.Value.Array.Length - 1; index >= 0; index--)
                    pending.Push((item.Value.Array[index], $"{item.Path}[{index.ToString(CultureInfo.InvariantCulture)}]"));
            }
        }
        ...
        out string? validationError)
    {
        ...
        foreach (var propertyName in value.Fields.Keys)
        {
            if (objectType.Fields.Any(field =>
                    string.Equals(field.Name, propertyName, StringComparison.OrdinalIgnoreCase)))
            {
                continue;
            }

            validationError = $"{context} contains unknown property '{propertyName}'.";
            return false;
        }
        ...
        out string? validationError)
    {
        ...
        foreach (var propertyName in value.Fields.Keys)
        {
            if (structural.Fields.Any(field =>
                    string.Equals(field.Name.Value, propertyName, StringComparison.OrdinalIgnoreCase)))
            {
                continue;
            }

            validationError =
                $"{context} contains unknown property '{propertyName}' for type '{structural.Id.Value}'.";
            return false;
        }
