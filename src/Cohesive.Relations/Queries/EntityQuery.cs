...
{
    /// <summary>
    /// Creates an entity-read request.
    /// </summary>
    public FieldSelection(IReadOnlyCollection<string>? fields = null) =>
        Fields = NormalizeFields(fields);

    /// <summary>
    /// Full-record read request.
    /// </summary>
    public static FieldSelection Full { get; } = new();

    /// <summary>
    /// An optional selected field subset, or <see langword="null" /> for a full read.
    /// </summary>
    public IReadOnlySet<string>? Fields { get; }

    /// <summary>
    /// Indicates whether the request projects a field subset.
    /// </summary>
    public bool HasFieldProjection => Fields is not null;

    /// <summary>
    /// Creates a projected-field request.
    /// </summary>
    public static FieldSelection ForFields(params string[] fields) => new(fields);
    ...
    static IReadOnlySet<string>? NormalizeFields(IReadOnlyCollection<string>? fields)
    {
        ...
        foreach (var field in fields)
        {
            if (string.IsNullOrWhiteSpace(field))
                throw new ArgumentException("Field-selection names must not be null, empty, or whitespace.", nameof(fields));
