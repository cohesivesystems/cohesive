...
The armed host receives an attributable fault plan through environment variables, writes one atomic JSON marker into a run-specific artifact directory, and blocks without committing additional lifecycle state. The external supervisor verifies that marker against the host PID before killing the actual process tree. Replacement hosts start without the fault plan and reconstruct only from canonical Process, progress, settlement, target, and provider authorities. This marker is observation evidence, not a second lifecycle authority.
