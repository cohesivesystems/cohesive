...
This slice prepares real provider changes for incremental materialization; it does not yet consume those changes into Elasticsearch. PostgreSQL logical-WAL binding and the Cosmos emulator envelope reader are the next integration boundary.
...
The vNext emulator proves local NoSQL gateway behavior but reports an Eventual account consistency level and does not support the production full-fidelity change-feed/continuous-backup contract. The Cosmos rebuild therefore uses the real Cosmos relation reader plus Cohesive's deterministic in-memory reconciliation pager. For canonical rebuild planning, a baseline-seeded attempt exposes a harness-only complete empty change interval. The harness declares a 30-minute convergence-freshness window so crash/restart drills over the evidence-heavy reference store do not turn that baseline source into a false lag failure. The deterministic Cosmos mutation envelopes preserve before images and source-transaction boundaries for the forthcoming emulator change-source interpretation, but they are not a claim that the emulator provides production full-fidelity change-feed semantics. Production incremental indexing remains bound to the stricter provider change-source contracts.
