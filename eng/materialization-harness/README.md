...
Location rows are conservatively acquired as tenant-bounded source sets because the current local lowerer cannot yet issue a lookup from a derived collection-element occurrence. This is an explicit bounded physical-plan choice, not a second endpoint-selection implementation: every candidate remains tenant-fenced and subject to fan-out and local-row limits, while the canonical interpreter alone decides which ordered stop is logically reachable. Location changes likewise use explicit bounded global invalidation until derived-occurrence lineage can compile an exact inverse route. The canonical fixture's behavioral tests cover stop reorder, location move, selected-stop deletion, storage-realization equivalence, and fail-closed bounded invalidation.
