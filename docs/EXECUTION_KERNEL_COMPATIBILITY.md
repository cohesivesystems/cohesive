...
| EK-11 — durable polling recurrence | Pass | `RepeatAcrossActivation` evaluates a typed progress value and Boolean continuation condition, admits at most one repeat between durable cuts, retains exact occurrence and unchanged-progress counts, and routes deterministically to Completed, Exhausted, or Stalled. Its optional typed state form persists the exact next-occurrence value in recurrence evidence, removes its duplicate token binding while parked at the recurrence cut, restores the binding before the next body entry, and rejects missing or contract-invalid state. The source frontend accepts a single state parameter and initial argument; direct fluent authoring expresses the same canonical members. The reference and Durable Task interpreters serialize, restore, validate, and resume the same state without a suspended host frame, ambient cursor store, or free graph cycle. | None in the provider-neutral reference and composed Durable Task profiles. Concrete Relation and Process-store adapters must pass the same recurrence and restore scenarios for their claimed realization. |
...
buffered inputs instead of retaining impossible `Buffered` state. An optional `CancellationFinalizerProcessNode`
retains the exact command intent, waits for exact closure of every propagated child, invokes one pinned finalizer
Process through its ordinary Request/Reply protocol, and reaches `Cancelled` only after an acknowledgement for the
same attempt; invalid acknowledgement or finalizer failure is terminal failure. Processes without the declaration
preserve immediate cancellation. A `RestartAttempt` recovery never resumes the abandoned continuation;
...
Object expressions take their field identities from readable semantic properties, including their configured JSON
names; constructor parameter names never become a parallel shape authority. A source-declared secondary constructor
is portable only when it delegates through an exactly resolvable, explicitly supplied `this(...)` chain that ends at
the compiler-declared property constructor. The generator composes parameter projections through that chain, so
renaming and reordering remain faithful. An authored constructor body without such a chain, unresolved delegation,
implicit/defaulted delegation arguments, or duplicate semantic field assignment fails with `COHPC004` instead of
guessing at runtime construction semantics.
...
`Cohesive.Processes.Contracts` roots the exact execution envelope, typed Process payload, and portable inspect,
status, explanation, and retained-trace observation contracts for code generation. The generated
`@cohesivesystems/processes` package preserves the closed node and AwaitMatch-clause unions, stable
...
| Reference transition interpretation | `TransitionReferenceInterpreter.Decide`, `DecideFullState`, `DecideSparse`, and explicit absent-subject `DecideCreation` → `TransitionDecision` plus `TransitionExecutionEvidence`, including complete initial-observation evidence |
...
| Canonical Process lifecycle control | `ProcessControlCommand`, `ProcessControlState`, `ProcessControlDecision`, `ProcessControlJsonSerializer`, `ProcessControlReferenceExecutor`, and `ProcessDurableRuntime` → protocol-neutral Inspect/Signal/Pause/Continue/RestartAttempt/Cancel/Terminate semantics, stable command identity and idempotency, exact attempt/revision fencing, replay and typed no-op receipts, safe-point lifecycle, attempt lineage, durable Signal admission, immediate or authored cancellation-finalization composition with explicit failure, and write-once attempt affinity; index-sync work still owns physical generation lifecycle |
| Canonical Process C# authoring | `GenerateProcessDefinition` + syntax-only `ProcessContext` is the primary human-facing frontend: ordinary C# `await`, pure locals, `if`/`else`, exact `switch`, named local branches, typed `AwaitMatch` result families, typed Fork/Join, bounded work, child invocation, recurrence, direct typed domain-event and Signal emission, one typed top-level cancellation-finalizer declaration, and terminal flow lower to canonical `ExecutionDefinitionDocument`. `OnCancellation` lowers directly to the existing `CancellationFinalizerProcessNode`, requires a protocol whose input contains the exact root input type, and introduces no control-flow edge or runtime callback. `EmitEvent` and `SendSignal` retain exact interaction contracts, portable payload/target expressions, stable node and continuation roles, and source attribution in the existing canonical nodes; envelope identity, correlation, causation, idempotency, authority, delivery, and provenance remain execution-derived canonical evidence. A typed wait's exhaustive type switch fuses directly into canonical clause continuations; its CLR base/case family, local value, and patterns are erased, while exact clause contracts, inputs, timer expressions, policies, identities, and provenance remain. A named typed child-outcome handler may be reused by multiple invocation sites: each use lowers to an occurrence-specific continuation with its own stable identity path and typed output binding, rather than a delegate reference or an implicit converged node. The four terminal roles within one invocation remain exhaustive and distinct. Pure locals fuse into the fixed portable expression closure; object field identities come from semantic properties, and secondary constructors are accepted only through exact `this(...)` projection chains or rejected with `COHPC004`. Callbacks, constructor bodies, and host-language state machines do not become Process nodes. `ProcessAuthoring.Create` + `ProcessBuilder<TInput,TResult>` remains an advanced generator/import/test escape hatch for the same closed node union and fingerprint. |
| Canonical Process semantics | `Cohesive.Processes.IR.ProcessDefinition`, `ProcessStaticCompiler`, `ProcessContinuationState`, `ProcessReferenceInterpreter`, `ProcessContinuationValidator`, `ProcessDurableCheckpoint`, `IProcessDurableStore`, and `ProcessDurableRuntime` → validated exact finite-activation plans, immutable multi-token continuations, deterministic operations/Fork/Join/waits/Requests/interactions, authored cancellation finalization after exact propagated-child closure, compatibility-first restore, exact activation/host-operation replay, atomic checkpoint/inbox/outbox/operation persistence, ambiguous exact mutation retry, and crash-testable CAS/fencing |
...
| Durable Task Process realization and admission | `DurableTaskProcessTargetProfile`, `DurableTaskProcessRealizationCompiler`, and `DurableTaskProcessRealizationPlan` → separate versioned planning and executable profiles that each dispose the complete canonical construct/guarantee catalogs, exact canonical-plan retention, requirement/node/link/evidence attribution, deterministic plan construction, and pre-plan rejection of unavailable semantics. Only an executable-profile realization may enter the worker catalog; planning evidence cannot authorize execution, and a canonical inventory addition without an executable disposition fails qualification. Authored cancellation finalization is composed through ordinary child sub-orchestration and exact Request/Reply recovery while lifecycle control remains `Cancelling`. |
...
| Process-invoked entity creation | Canonical `TransitionSubjectCreation` + `EntityTransitionProcessOperationAdapter` + `IEntityTransitionOperationRepository` → exact input-derived version-zero state, portable entity-definition validation, atomic absence fence, state/receipt/emission handoff, exact-occurrence replay, and authority-scoped natural idempotency across occurrences by exact Transition/subject/input intent. A semantic replay returns the original receipt and envelopes so target deduplication retains one identity; changed intent conflicts, and a pre-existing subject without an attributable atomic creation receipt remains rejected. Concurrent exact creates converge on one mutation and receipt without introducing a parallel Process create node or general upsert semantics. |
| Durable Task execution and monitoring | `DurableTaskSequentialProcessPlanCatalog` + `DurableTaskSequentialProcessOrchestrator` → exact-plan execution for Transition, Relation/Query, Request, durable after-origin domain-event emission, Signal send to a Process token, Choice, Match, bounded Fork/Join, AwaitMatch, Timer, child Process, bounded partition, bounded recurrence, Durable Cut, Return, and Fail through the standalone SDK. Bounded activities materialize host-operation, domain-event publication, and Signal-target evidence around the canonical reference interpreter; concurrent branch Requests and child sub-orchestrations retain canonical target admission, branch selection, child lineage, and `Propagate`/`Detach` cancellation policy. Persisted canonical wait and due-instant state remains semantic authority while replayable Durable Task external events and timers provide physical stimuli. Domain-event publication sends the unchanged canonical envelope through an activity only for durable after-origin visibility. `IDomainEventPublisherResolver` selects an exact-contract publisher whose declared capabilities guarantee target deduplication by authority scope, exact contract, and canonical idempotency key; plans with direct event nodes fail catalog admission without that guarantee, dynamic host events fail before publication I/O, and retained acknowledgement evidence survives replay and Continue-as-new without claiming physical exactly-once or atomic-with-origin delivery. The exact canonical Signal envelope is routed unchanged, but recipient target validation, arbitration, and early/late/stale/duplicate/missing-target/consumed dispositions remain reference-interpreter decisions; provider delivery is not canonical admission. Co-ready AwaitMatch input/timer stimuli enter one canonical activation; the reference interpreter retains guard, priority, clause tie-break, winner, and policy authority. Bound Requests reuse `DurableOperationReferenceExecutor` for stable identities, claims, dispatch markers, retry, reconciliation, acknowledgement, and Reply admission; at-least-once activities require target deduplication or natural idempotence and authored recovery that cannot yet run fails closed with its canonical ledger. Partition and recurrence bounds fail closed, while Continue-as-new carries the complete canonical cut state without becoming semantic authority. Exact definition tuples survive replay, unsupported constructs fail executable qualification before catalog admission, and `ProcessExecutionStatusProjector` publishes safe protocol-neutral `ExecutionStatus` custom status from canonical continuation/control state with terminal detail redacted. Scheduler custom status omits commands, receipts, interactions, operation ledgers, wait keys, traces, and input/output payloads. Every newly executed finite activation projects the existing payload-safe `NormalizedExecutionTrace` from its authoritative canonical decision; results retain those traces in activation order across replay, attempt replacement, and Continue-as-new, while pre-retention gaps are not fabricated from provider history. New schedules add a versioned immutable payload-free Scheduler tag projection for logical Process and exact definition discovery; mutable lifecycle/location stays in canonical custom status. `DurableTaskProcessExecutionRepository` implements provider-neutral physical and trusted logical reads through `IProcessExecutionRepository`, returns exact canonical definition and status without projecting fetched Process input/output or provider failure bodies, validates physical/start/status/tag affinity, and retains the Core query-client constructor only for migrated historical wire shapes. Physical task-hub identity and logical Process identity remain explicit and distinct; trusted authority scope plus logical identity deterministically derives one exact current lookup without a page scan. The pinned emulator proves tag round-trip plus live and terminal logical repository retrieval. `IProcessExecutionTraceRepository` performs opt-in physical or trusted logical reads of terminal canonical results, distinguishes missing/active/available/terminal-without-artifact states, and returns the strict versioned `ProcessExecutionTraceArtifact` without its physical repository key. The artifact preserves exact definition/logical-instance affinity, normalized activation order, and explicit missing-prefix coverage; the migration-only Core reader remains unsupported rather than deriving semantics from provider history. `IProcessExecutionValueRepository` is a separate authority-scoped opt-in for potentially sensitive canonical start and terminal `PortableValue` evidence; it preserves explicit active/available/terminal-without-artifact states while generic monitoring remains payload-free. `IProcessExecutionExplainRepository` returns the existing canonical explain artifact through physical or trusted authority-scope/logical identity; the Durable Task composition resolves the exact deployed plan, projects every source-inventory realization disposition, includes safe status and at most the latest current-attempt trace, and reports legacy coverage or missing terminal artifacts as structured warnings without consulting provider history or exposing physical identity. `MapProcessExecutionInspectApi`, `MapProcessExecutionExplainApi`, and `MapProcessExecutionTracesApi` project the existing catalog handles into authorized ASP.NET GETs that accept only logical identity from the route and share one trusted authority-scope convention. Inspect returns only retained canonical status in `ExecutionControlResult`, concealing missing and pending-without-status observations alike; explain and retained traces return exact canonical JSON bytes. Trace repository states map through declared opaque success, not-found, conflict, and precondition-failed outcomes. Lifecycle mutation bindings, live trace streaming, richer dashboard presentation, and history-event normalization remain incomplete. Reply emission, atomic-with-origin domain-event publication, activation-local and non-Process Signal targets, external Signal adapters, and complete observability are not yet promoted. |
