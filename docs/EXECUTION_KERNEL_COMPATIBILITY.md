...
| Process-invoked entity creation | Canonical `TransitionSubjectCreation` + `EntityTransitionProcessOperationAdapter` + `IEntityTransitionOperationRepository` → exact input-derived version-zero state, portable entity-definition validation, atomic absence fence, state/receipt/emission handoff, replay, conflict, and crash-cut recovery without a parallel Process create node |
