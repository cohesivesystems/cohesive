...
| DurableTask monitoring | `DurableTaskProcessExecutionRepository` → authority-neutral task-hub query projection only; the delegate orchestration host and execution registration surface are not shipped. A parallel canonical interpreter is [accepted direction](decisions/durable-task-process-interpreter.md), not implemented capability. |
