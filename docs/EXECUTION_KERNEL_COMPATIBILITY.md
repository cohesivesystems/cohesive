...
| Durable Task execution and monitoring | `DurableTaskSequentialProcessPlanCatalog` + `DurableTaskSequentialProcessOrchestrator` → exact-plan sequential execution for Transition, Relation/Query, Request, Choice, Match, Durable Cut, Return, and Fail through the standalone SDK; bounded activities materialize host-operation evidence into the canonical reference interpreter, exact definition tuples survive replay, and unsupported constructs fail catalog admission. `DurableTaskProcessExecutionRepository` remains the authority-neutral task-hub monitoring projection. Full Request recovery, waits, concurrency, control, lifecycle, and observability are not yet promoted. |
