...
interaction contracts and runtime envelopes now provide the shared event, Request, Signal, and Reply vocabulary,
but the legacy Process and Storage runtimes do not yet enforce that vocabulary durably. EK-09 is Partial:
...
| EK-02 — durable human review | Partial | Canonical Request, Signal, and Reply contracts can declare portable payloads, exact semantic continuation addresses, and typed timeout or cancellation outcomes. Envelopes carry stable identity and provenance. The interaction reader does not yet resolve those addresses against Process/Transition definitions or live continuation state. `WaitNode` still produces the legacy `Waiting` checkpoint, while local and DurableTask runtimes buffer early keyed inputs and DurableTask supplies timers. | Closed `AwaitMatch`, definition/node linking, durable registration plus timer arming, and runtime admission/claim/consume, deduplication, arbitration, and late/stale enforcement over canonical envelopes. |
| EK-03 — vendor/manual fulfillment | Partial | Canonical Request contracts declare terminal variants, response targets, retry and ambiguous-outcome semantics, late/stale/duplicate policies, and retention; Request and Reply envelopes carry stable correlation and idempotency identity. Typed legacy effect handlers, transient retry, continuation freshness checks, and dead-lettering also exist. | Durable Request/Reply ledger integration, vendor/manual arbitration, attempt and acknowledgement evidence, and runtime enforcement preventing late or stale results from advancing the wrong continuation. |
...
| EK-06 — durable effect crash matrix | Partial | Canonical envelopes provide stable emission, correlation, causation, and idempotency identity plus explicit delivery, retry, ambiguous-outcome, reconciliation, and response obligations. Transition effects can also be committed through the existing storage outbox, and legacy checkpoints distinguish pending and executed effects and retain dead letters. | A durable operation ledger that applies the canonical identities and obligations to request attempts, claims, acknowledgements, reconciliation, and completion across every crash boundary. |
