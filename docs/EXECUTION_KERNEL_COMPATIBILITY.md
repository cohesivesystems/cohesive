...
| Durable Task Process realization planning | `DurableTaskProcessTargetProfile`, `DurableTaskProcessRealizationCompiler`, and `DurableTaskProcessRealizationPlan` → versioned planning-only Durable Task dispositions for the complete current construct/guarantee catalogs, exact canonical-plan retention, requirement/node/link/evidence attribution, deterministic plan construction, and pre-plan rejection of unavailable semantics. No Durable Task Process execution host is shipped. |
...
| DurableTask monitoring | `DurableTaskProcessExecutionRepository` → authority-neutral task-hub query projection only; the delegate orchestration host and execution registration surface are not shipped. A parallel canonical interpreter is [accepted direction](decisions/durable-task-process-interpreter.md); planning is implemented, execution is not. |
