...
| Canonical Process C# authoring | `ProcessAuthoring.Create` + `ProcessBuilder<TInput,TResult>` → canonical `ExecutionDefinitionDocument`; typed values, bindings, obligations, all closed-union nodes, deterministic owner-relative identities, immediate member-path capture, and `ExecutionSourceMap` attribution without retained callbacks. `GenerateProcessDefinition` additionally lowers syntax-only C# `await`, pure locals, `if`/`else`, exact `switch`, and `return` into the same builder and canonical expression closure; pure locals do not become Process nodes. |
